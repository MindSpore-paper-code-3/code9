...
def preprocess_fn(image, box, mask, mask_shape, is_training):
    ...
    def _data_aug(image, box, mask, mask_shape, is_training):
        ...
        gt_iscrowd = box[:, 5]
        gt_mask = mask.copy()
        gt_mask = np.frombuffer(gt_mask, dtype=np.uint8)
        n, h, w = mask_shape
...
def create_coco_label(is_training):
    ...
    for ind, img_id in enumerate(image_ids):
        ...
        if not is_training:
            ...
            masks[image_path] = np.zeros([1, 1, 1], dtype=np.bool_).tobytes()
            ...
            for label in anno:
                ...
                if class_name in train_cls:
                    ...
                    if label['iscrowd'] and (m.shape[0] != image_height or m.shape[1] != image_width):
                        m = np.ones([image_height, image_width], dtype=np.bool_)
            ...
            if annos:
                ...
                instance_masks = np.stack(instance_masks, axis=0).astype(np.bool_)
                ...
                image_anno_dict[image_path] = np.array([0, 0, 0, 0, 0, 1])
                masks[image_path] = np.zeros([1, image_height, image_width], dtype=np.bool_).tobytes()
