...
def train():
    ...
    config.mindrecord_path = os.path.abspath(os.path.join(dirname, config.mindrecord_path))
    ...
    if config.device_num > 1:
        params_num = len(net.trainable_params())
        ms.set_auto_parallel_context(all_reduce_fusion_config=[params_num // 2, params_num // 3 * 2, params_num - 1])
