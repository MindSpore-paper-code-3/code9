    ...
    def __init__(self, config, batch_size, num_bboxes, add_gt_as_proposals):
        ...
        if self.use_ambigous_sample:
            self.check_neg_mask = Tensor(
                np.array(np.ones(self.num_expected_neg - self.num_expected_pos - self.num_expected_amb),
                         dtype=np.bool_))
        check_neg_mask_ignore_end = np.array(np.ones(self.num_expected_neg), dtype=np.bool_)
