...
# 运行评估示例
用法：bash run_eval.sh [DATASET_PATH] [CHECKPOINT_PATH] [CONFIG_PATH] [DEVICE_ID](optional)
...
注：为了获得更好的性能，在Ascend平台建议batch_size设置为NPU核的倍数。
...
- **Q: 如何解决910B硬件上因ReduceMean、BiasAddGrad等累加算子导致的内存不足？**

  **A**： 建议在`train.py`中添加`ms.set_context(ascend_config={"atomic_clean_policy": 0})`，如果还是没有解决问题，请到[MindSpore社区](https://gitee.com/mindspore/mindspore/issues)提issue。

- **Q: 遇到`out of memory`如何解决？**

  **A**： 在`*.yaml`中配置合适大小的`max_device_memory`，例如：`max_device_memory: "20GB"`。
